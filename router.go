...
// 是否是具体的主题
func isSpecifTopic(topic string) bool {
	if len(topic) == 0 {
		return true
	}
	for _, c := range topic {
...
type router struct {
	sync.RWMutex
	specifCallback map[string]MessageHandler // 那些已指定到具体主题的回调. 也就是topic里不包含 +, # 这样的通配符的
	routes         *list.List
...
func newRouter() (*router, chan bool) {
	router := &router{
		specifCallback: make(map[string]MessageHandler, 1024),
		routes:         list.New(),
...
func (r *router) addRoute(topic string, callback MessageHandler) {
	r.Lock()
	defer r.Unlock()
	if isSpecifTopic(topic) {
		r.specifCallback[topic] = callback
	}
	...
	go func() {
		for {
			...
			case message := <-messages:
				{ // 这块属于自己添加的, 看看是否匹配到具体的主题
					r.RLock()
					callback, ok := r.specifCallback[message.TopicName]
					r.RUnlock()
					if ok {
						// fmt.Println("匹配到具体topic=", message.TopicName)
						if order {
							callback(client, messageFromPublish(message))
						} else {
							go callback(client, messageFromPublish(message))
						}
